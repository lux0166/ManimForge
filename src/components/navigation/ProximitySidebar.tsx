...
const Dash = ({
  ...
  return (
    <button
      ...
      title={section.label}
      className="group flex h-2 w-[110px] items-center border-0 bg-transparent p-0 outline-none cursor-pointer"
      onClick={() => onSelect(section.id)}
    >
      <motion.span
        className={`block transition-colors duration-150 ease-out group-focus-visible:ring-1 group-focus-visible:ring-white ${
          active ? "bg-white" : preset.className
        }`}
        style={{
...
export function ProximitySidebar({
  ...
  const selectSection = useCallback(
    ...
    [onSectionClick, pulseDash, shouldReduceMotion]
  );
