...
const WORKSPACE = { key: "manimforge", name: "Academa Studio", monogram: "A" };
...
const DEFAULT_RECENTS: SidebarRecent[] = [
  { id: "video-2", label: "Video 2", prompt: "Show a tiny neural network making one prediction and then learning from its error." },
  { id: "video-1", label: "Video 1", prompt: "Fourier Transform decomposition of a square wave" },
  { id: "sorting-vis", label: "QuickSort Partitioning", prompt: "Animate QuickSort pivot selection and array swapping" },
  { id: "calculus-riemann", label: "Riemann Integral Sums", prompt: "Approximating area under curve with rectangles" },
];
  ...
  return createPortal(
    ...
    >
      <GlideMenu className="flex flex-col gap-px" highlightClassName="inset-x-0 rounded-[8px] bg-hover-2">
        ...
        {[
          { label: "New project", icon: <IconPlusMedium size={16} /> },
...
export default function SidebarNav({
  ...
  return (
    <aside
      ...
      className={`relative flex shrink-0 overflow-hidden border-r border-line transition-[width] ${fill ? "h-full" : "h-[600px]"} ${className}`}
      style={{
    ...
    >
      <div className="flex min-h-0 w-[224px] shrink-0 flex-col bg-surface-subtle">
          ...
          <GlideGroup>
            {visibleRecents.map((item) => {
              ...
              return (
                ...
                >
                  <span className={`sidebar-copy min-w-0 flex-1 truncate text-[14px] font-medium ${active ? "text-ink font-semibold" : "text-ink-2"}`}>
                    {item.label}
                  </span>
                </button>
              );
            ...
            {query && visibleRecents.length === 0 && (
              <div className="sidebar-copy mx-2 px-2 py-2 text-[12.5px] text-ink-3">No videos found</div>
            )}
          </GlideGroup>
        </div>

        <div className="sidebar-copy mx-2 mt-3 w-[208px] border-t border-line pb-3 pt-3">
          <button
            type="button"
            onClick={onFooterClick ?? onNewChat}
            className="flex h-9 w-full items-center justify-center gap-1.5 rounded-control bg-purple-600/20 text-[13px] font-medium text-purple-300 transition-[background-color,transform] duration-150 hover:bg-purple-600/30 active:scale-[0.98]"
          >
            {footerIcon}
            {footerLabel}
          </button>
